...
import { JEONGHWA_HTML } from "./jeonghwa-content";

const PARA_REGEX = /<p class="jeonghwa-p"[^>]*>[\s\S]*?<\/p>/g;
...
function getSections(html: string): Array<{ visible: string; rest: string }> {
  const parts = html.split(/<hr class="jeonghwa-hr"\s*\/>/);
...
const contentClass =
  "prose prose-slate max-w-none text-[#2D2D2D] [&_.jeonghwa-p]:mb-4 [&_.jeonghwa-p]:leading-relaxed [&_strong]:font-bold [&_span]:align-baseline [&_.jeonghwa-hr]:my-6 [&_.jeonghwa-hr]:border-slate-300 [&_.jeonghwa-ul]:my-4 [&_.jeonghwa-ul]:list-disc [&_.jeonghwa-ul]:pl-6 [&_.jeonghwa-ul]:space-y-1";

export default function JeonghwaPage() {
  const sections = useMemo(() => getSections(JEONGHWA_HTML), []);
  ...
  const handleShare = async () => {
    ...
    const title = `${TYPE_DISPLAY_NAMES.정} 성격유형`;
    try {
      if (navigator.share) {
    ...
    <main className="relative min-h-screen flex flex-col items-center p-6 sm:p-8 overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-2xl flex flex-col gap-6">
        ...
        <article className="rounded-2xl bg-white shadow-[0_4px_14px_rgba(0,0,0,0.08),0_2px_6px_rgba(0,0,0,0.04)] overflow-hidden">
          <div className="p-6 sm:p-8 border-b border-[#E8E5DD] flex flex-col sm:flex-row items-center sm:items-end gap-4">
            ...
            <div className="flex flex-col items-center gap-4 shrink-0 w-full sm:w-auto">
              ...
              <div className="relative w-[160px] h-[160px] sm:w-[200px] sm:h-[200px] flex items-center justify-center flex-shrink-0">
                <Image
                  src={TYPE_CHARACTER_IMAGES.정}
                  alt={TYPE_DISPLAY_NAMES.정}
                  fill
          ...
          <div className="p-6 sm:p-8 jeonghwa-content">
            <div className={contentClass}>
              {sections.map((sec, i) => (
                <div key={i} className="jeonghwa-section">
                  <div
                    className="jeonghwa-section-visible"
                    dangerouslySetInnerHTML={{ __html: sec.visible }}
                  />
                  {sec.rest ? (
                    <>
                      <div
                        className={`jeonghwa-section-rest ${expanded[i] ? "jeonghwa-section-rest-expanded" : ""}`}
                        style={
                          expanded[i]
                            ? undefined
                            : {
                      ...
                      >
                        <div
                          className="jeonghwa-section-rest-inner"
                  ...
                  {i < sections.length - 1 && (
                    <hr className="jeonghwa-hr my-6 border-slate-300" />
                  )}
                </div>
              ))}
            </div>
