...
import { SHINGEUM_HTML } from "./shingeum-content";

const PARA_REGEX = /<p class="shingeum-p"[^>]*>[\s\S]*?<\/p>/g;
...
function getSections(html: string): Array<{ visible: string; rest: string }> {
  const parts = html.split(/<hr class="shingeum-hr"\s*\/>/);
...
const contentClass =
  "prose prose-slate max-w-none text-[#2D2D2D] [&_.shingeum-p]:mb-4 [&_.shingeum-p]:leading-relaxed [&_strong]:font-bold [&_span]:align-baseline [&_.shingeum-hr]:my-6 [&_.shingeum-hr]:border-slate-300 [&_.shingeum-ul]:my-4 [&_.shingeum-ul]:list-disc [&_.shingeum-ul]:pl-6 [&_.shingeum-ul]:space-y-1";

export default function ShingeumPage() {
  const sections = useMemo(() => getSections(SHINGEUM_HTML), []);
  ...
  const handleShare = async () => {
    ...
    const title = `${TYPE_DISPLAY_NAMES.신} 성격유형`;
    try {
      if (navigator.share) {
    ...
    <main className="relative min-h-screen flex flex-col items-center p-6 sm:p-8 overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-2xl flex flex-col gap-6">
        ...
        <article className="rounded-2xl bg-white shadow-[0_4px_14px_rgba(0,0,0,0.08),0_2px_6px_rgba(0,0,0,0.04)] overflow-hidden">
          <div className="p-6 sm:p-8 border-b border-[#E8E5DD] flex flex-col sm:flex-row items-center sm:items-end gap-4">
            ...
            <div className="flex flex-col items-center gap-4 shrink-0 w-full sm:w-auto">
              ...
              <div className="relative w-[160px] h-[160px] sm:w-[200px] sm:h-[200px] flex items-center justify-center flex-shrink-0">
                <Image
                  src={TYPE_CHARACTER_IMAGES.신}
                  alt={TYPE_DISPLAY_NAMES.신}
                  fill
          ...
          <div className="p-6 sm:p-8 shingeum-content">
            <div className={contentClass}>
              {sections.map((sec, i) => (
                <div key={i} className="shingeum-section">
                  <div
                    className="shingeum-section-visible"
                    dangerouslySetInnerHTML={{ __html: sec.visible }}
                  />
                  {sec.rest ? (
                    <>
                      <div
                        className={`shingeum-section-rest ${expanded[i] ? "shingeum-section-rest-expanded" : ""}`}
                        style={
                          expanded[i]
                            ? undefined
                            : {
                      ...
                      >
                        <div
                          className="shingeum-section-rest-inner"
                  ...
                  {i < sections.length - 1 && (
                    <hr className="shingeum-hr my-6 border-slate-300" />
                  )}
                </div>
              ))}
            </div>
