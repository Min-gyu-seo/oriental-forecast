...
import { GYEONGGEUM_HTML } from "./gyeonggeum-content";

const PARA_REGEX = /<p class="gyeonggeum-p"[^>]*>[\s\S]*?<\/p>/g;
...
function getSections(html: string): Array<{ visible: string; rest: string }> {
  const parts = html.split(/<hr class="gyeonggeum-hr"\s*\/>/);
...
const contentClass =
  "prose prose-slate max-w-none text-[#2D2D2D] [&_.gyeonggeum-p]:mb-4 [&_.gyeonggeum-p]:leading-relaxed [&_strong]:font-bold [&_span]:align-baseline [&_.gyeonggeum-hr]:my-6 [&_.gyeonggeum-hr]:border-slate-300 [&_.gyeonggeum-ul]:my-4 [&_.gyeonggeum-ul]:list-disc [&_.gyeonggeum-ul]:pl-6 [&_.gyeonggeum-ul]:space-y-1";

export default function GyeonggeumPage() {
  const sections = useMemo(() => getSections(GYEONGGEUM_HTML), []);
  ...
  const handleShare = async () => {
    ...
    const title = `${TYPE_DISPLAY_NAMES.경} 성격유형`;
    try {
      if (navigator.share) {
    ...
    <main className="relative min-h-screen flex flex-col items-center p-6 sm:p-8 overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-2xl flex flex-col gap-6">
        ...
        <article className="rounded-2xl bg-white shadow-[0_4px_14px_rgba(0,0,0,0.08),0_2px_6px_rgba(0,0,0,0.04)] overflow-hidden">
          ...
          <div className="p-6 sm:p-8 gyeonggeum-content">
            <div className={contentClass}>
              {sections.map((sec, i) => (
                <div key={i} className="gyeonggeum-section">
                  <div
                    className="gyeonggeum-section-visible"
                    dangerouslySetInnerHTML={{ __html: sec.visible }}
                  />
                  {sec.rest ? (
                    <>
                      <div
                        className={`gyeonggeum-section-rest ${expanded[i] ? "gyeonggeum-section-rest-expanded" : ""}`}
                        style={
                          expanded[i]
                            ? undefined
                            : {
                      ...
                      >
                        <div
                          className="gyeonggeum-section-rest-inner"
                  ...
                  {i < sections.length - 1 && (
                    <hr className="gyeonggeum-hr my-6 border-slate-300" />
                  )}
                </div>
              ))}
            </div>
