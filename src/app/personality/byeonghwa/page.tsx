...
import { BYEONGHWA_HTML } from "./byeonghwa-content";

const PARA_REGEX = /<p class="byeonghwa-p"[^>]*>[\s\S]*?<\/p>/g;
...
function getSections(html: string): Array<{ visible: string; rest: string }> {
  const parts = html.split(/<hr class="byeonghwa-hr"\s*\/>/);
...
const contentClass =
  "prose prose-slate max-w-none text-[#2D2D2D] [&_.byeonghwa-p]:mb-4 [&_.byeonghwa-p]:leading-relaxed [&_strong]:font-bold [&_span]:align-baseline [&_.byeonghwa-hr]:my-6 [&_.byeonghwa-hr]:border-slate-300 [&_.byeonghwa-ul]:my-4 [&_.byeonghwa-ul]:list-disc [&_.byeonghwa-ul]:pl-6 [&_.byeonghwa-ul]:space-y-1";

export default function ByeonghwaPage() {
  const sections = useMemo(() => getSections(BYEONGHWA_HTML), []);
  ...
  const handleShare = async () => {
    ...
    const title = `${TYPE_DISPLAY_NAMES.병} 성격유형`;
    try {
      if (navigator.share) {
    ...
    <main className="relative min-h-screen flex flex-col items-center p-6 sm:p-8 overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-2xl flex flex-col gap-6">
        ...
        <article className="rounded-2xl bg-white shadow-[0_4px_14px_rgba(0,0,0,0.08),0_2px_6px_rgba(0,0,0,0.04)] overflow-hidden">
          ...
          <div className="p-6 sm:p-8 byeonghwa-content">
            <div className={contentClass}>
              {sections.map((sec, i) => (
                <div key={i} className="byeonghwa-section">
                  <div
                    className="byeonghwa-section-visible"
                    dangerouslySetInnerHTML={{ __html: sec.visible }}
                  />
                  {sec.rest ? (
                    <>
                      <div
                        className={`byeonghwa-section-rest ${expanded[i] ? "byeonghwa-section-rest-expanded" : ""}`}
                        style={
                          expanded[i]
                            ? undefined
                            : {
                      ...
                      >
                        <div
                          className="byeonghwa-section-rest-inner"
                  ...
                  {i < sections.length - 1 && (
                    <hr className="byeonghwa-hr my-6 border-slate-300" />
                  )}
                </div>
              ))}
            </div>
