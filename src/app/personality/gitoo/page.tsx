...
import { GITOO_HTML } from "./gitoo-content";

const PARA_REGEX = /<p class="gitoo-p"[^>]*>[\s\S]*?<\/p>/g;
...
function getSections(html: string): Array<{ visible: string; rest: string }> {
  const parts = html.split(/<hr class="gitoo-hr"\s*\/>/);
...
const contentClass =
  "prose prose-slate max-w-none text-[#2D2D2D] [&_.gitoo-p]:mb-4 [&_.gitoo-p]:leading-relaxed [&_strong]:font-bold [&_span]:align-baseline [&_.gitoo-hr]:my-6 [&_.gitoo-hr]:border-slate-300 [&_.gitoo-ul]:my-4 [&_.gitoo-ul]:list-disc [&_.gitoo-ul]:pl-6 [&_.gitoo-ul]:space-y-1";

export default function GitooPage() {
  const sections = useMemo(() => getSections(GITOO_HTML), []);
  ...
  const handleShare = async () => {
    ...
    const title = `${TYPE_DISPLAY_NAMES.기} 성격유형`;
    try {
      if (navigator.share) {
    ...
    <main className="relative min-h-screen flex flex-col items-center p-6 sm:p-8 overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-2xl flex flex-col gap-6">
        ...
        <article className="rounded-2xl bg-white shadow-[0_4px_14px_rgba(0,0,0,0.08),0_2px_6px_rgba(0,0,0,0.04)] overflow-hidden">
          <div className="p-6 sm:p-8 border-b border-[#E8E5DD] flex flex-row items-end gap-4">
            ...
            <div className="flex flex-col items-center gap-4 shrink-0">
              <p className="text-2xl font-bold text-[#2D2D2D] text-center">
                내실 있는 실속파, 포근한 실질적 관리자
              </p>
              <div className="relative w-[200px] h-[200px] flex items-center justify-center">
                <Image
                  src={TYPE_CHARACTER_IMAGES.기}
                  alt={TYPE_DISPLAY_NAMES.기}
                  fill
          ...
          <div className="p-6 sm:p-8 gitoo-content">
            <div className={contentClass}>
              {sections.map((sec, i) => (
                <div key={i} className="gitoo-section">
                  <div
                    className="gitoo-section-visible"
                    dangerouslySetInnerHTML={{ __html: sec.visible }}
                  />
                  {sec.rest ? (
                    <>
                      <div
                        className={`gitoo-section-rest ${expanded[i] ? "gitoo-section-rest-expanded" : ""}`}
                        style={
                          expanded[i]
                            ? undefined
                            : {
                      ...
                      >
                        <div
                          className="gitoo-section-rest-inner"
                  ...
                  {i < sections.length - 1 && (
                    <hr className="gitoo-hr my-6 border-slate-300" />
                  )}
                </div>
              ))}
            </div>
