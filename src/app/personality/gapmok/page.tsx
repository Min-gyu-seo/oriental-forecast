...
import { GAPMOK_HTML } from "./gapmok-content";

const PARA_REGEX = /<p class="gapmok-p"[^>]*>[\s\S]*?<\/p>/g;
...
function getSections(html: string): Array<{ visible: string; rest: string }> {
  const parts = html.split(/<hr class="gapmok-hr"\s*\/>/);
...
const contentClass =
  "prose prose-slate max-w-none text-[#2D2D2D] [&_.gapmok-p]:mb-4 [&_.gapmok-p]:leading-relaxed [&_strong]:font-bold [&_span]:align-baseline [&_.gapmok-ul]:my-4 [&_.gapmok-ul]:list-disc [&_.gapmok-ul]:pl-6 [&_.gapmok-ul]:space-y-1";

export default function GapmokPage() {
  const sections = useMemo(() => getSections(GAPMOK_HTML), []);
  ...
  const handleShare = async () => {
    ...
    const title = `${TYPE_DISPLAY_NAMES.갑} 성격유형`;
    try {
      if (navigator.share) {
    ...
    <main className="relative min-h-screen flex flex-col items-center p-6 sm:p-8 overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-2xl flex flex-col gap-6">
        ...
        <article className="rounded-2xl bg-white shadow-[0_4px_14px_rgba(0,0,0,0.08),0_2px_6px_rgba(0,0,0,0.04)] overflow-hidden">
          ...
          <div className="p-6 sm:p-8 gapmok-content">
            <div className={contentClass}>
              {sections.map((sec, i) => (
                <div key={i} className="gapmok-section">
                  <div
                    className="gapmok-section-visible"
                    dangerouslySetInnerHTML={{ __html: sec.visible }}
                  />
                  {sec.rest ? (
                    <>
                      <div
                        className={`gapmok-section-rest ${expanded[i] ? "gapmok-section-rest-expanded" : ""}`}
                        style={
                          expanded[i]
                            ? undefined
                            : {
                      ...
                      >
                        <div
                          className="gapmok-section-rest-inner"
                  ...
                  {i < sections.length - 1 && (
                    <hr className="gapmok-hr my-6 border-slate-300" />
                  )}
                </div>
              ))}
            </div>
