...
export default function MyTypeResultPage() {
  ...
  useEffect(() => {
    try {
      const raw = sessionStorage.getItem(MY_TYPE_ANSWERS_KEY);
      const answers: Record<string, number> = raw ? JSON.parse(raw) : {};
      setTypes(getTendencyTypes(MY_TYPE_QUESTIONS, answers));
      setTenStemScores(getTenStemScores(MY_TYPE_QUESTIONS, answers));
    } catch {
      setTypes([]);
    }
  }, []);
